...
settings = MarketSettings(nr_of_hours=1, offer_type="energyBudget", prod_diff="noPref", market_design=md)
# set model name
...
# see the result object before computing relevant quantities
result.Ln
result.Gn
result.Pn
result.Tnm[0]  # for p2p this is a list of length nr_of_h
result.shadow_price[0]  # for p2p this is a list of length nr_of_h

# compute quantities
result.compute_output_quantities(settings, agent_data)
result.settlement
result.QoE
result.social_welfare_h

# produce market clearing plot and store it at a certain location as png or pdf
result.plot_market_clearing(period=0, settings=settings, agent_data=agent_data,
                            outfile="./figures/p2p_clearing.png")
