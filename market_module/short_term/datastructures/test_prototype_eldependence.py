...
elec_price = [1.0, 1.0, 1.0, 1.0, 2.0, 5.0, 1.0, 3.0, 1.0, 10.0, 2.0, 0.5]

# settings for electricity dependence
settings = MarketSettings(nr_of_hours=12, offer_type="simple", prod_diff="noPref", market_design=md,
                          network_type="direction", el_dependent=True,
                          el_price=elec_price)


name = "test_" + str(settings.market_design) + "_" + str(settings.offer_type) + "_" + str(settings.product_diff) + \
       "_" + str(settings.network_type) + "_el=" + str(settings.el_dependent)
print(name)
...
#
chps = ["producer_1", "prosumer_2"]
chpData = {"producer_1": {"alpha": 10.0, "r": 0.6, "rho_H": 0.8, "rho_E": 0.5}}

agent_data = AgentData(settings=settings, name=agent_ids, a_type=agent_types,
                       gmin=gmin, gmax=gmax,
                       lmin=lmin, lmax=lmax,
                       cost=cost, util=util,
                       co2=co2_emissions, is_chp=chps, chp_pars=chpData)
# have a look
print(agent_data.chp_params)
print(agent_data.cost)
